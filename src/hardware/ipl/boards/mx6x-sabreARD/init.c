...
static void init_fec_clk_125(void)
{
    _Uint32t val;
    /* Disable CCM_ANALOG_PLL_ENET */
    val = in32(MX6X_ANATOP_BASE + MX6X_ANATOP_PLL8_ENET);
    val &= ~ANATOP_PLL8_ENET_ENABLE;
    out32(MX6X_ANATOP_BASE + MX6X_ANATOP_PLL8_ENET, val);

    /* Enable the ethernet reference clock output as 125MHz*/
    val = ANATOP_PLL8_ENET_ENABLE | ANATOP_PLL8_ENET_REF_ENET_125M;
    out32(MX6X_ANATOP_BASE + MX6X_ANATOP_PLL8_ENET, val);
...
void init_clocks(void)
{
    ...
    /* Only reset 528PFD2 on the Q/D chip, as done by the 
       Freescale/NXP "ENGR00235821 mx6: correct work flow of PFDs" patch 
    */
    if ( get_imx6_type() == MX6_CHIP_TYPE_QUAD_OR_DUAL)
    {
    ...
    /* For Dual Lite TO1.2 cpu, select 125MHz */
    if (( get_imx6_rev() == MX6_CHIP_REV_1_2 ) &&
        (get_imx6_type() == MX6_CHIP_TYPE_DUAL_LITE_OR_SOLO))
    {
        init_fec_clk_125();
    }
...
void init_pinmux_dual_lite_solo()
{
	...
	set_mux_cfg(SWMUX_SDL_CSI0_DAT10, MUX_CTL_MUX_MODE_ALT3);	/* 20E_004C *//* IOMUXC_SW_MUX_CTL_PAD_CSI0_DATA10 */
	set_pad_cfg(SWPAD_SDL_CSI0_DAT10, MX6Q_PAD_SETTINGS_UART);	/* 20E_0360 *//* SW_PAD_CTL_PAD_CSI0_DATA10 */
	set_pin_as_input(SWINPUT_SDL_UART1_IPP_UART_RXD_MUX, 0x00);	/* 20E_08FC *//* IOMUXC_UART1_UART_RX_DATA_SELECT_INPUT */

	/* UART1 RX - pin CSI0_DAT11 */
	set_mux_cfg(SWMUX_SDL_CSI0_DAT11, MUX_CTL_MUX_MODE_ALT3);	/* 20E_0050 *//* IOMUXC_SW_MUX_CTL_PAD_CSI0_DATA11 */
	set_pad_cfg(SWPAD_SDL_CSI0_DAT11, MX6Q_PAD_SETTINGS_UART);	/* 20E_0364 *//* SW_PAD_CTL_PAD_CSI0_DATA11 */
	set_pin_as_input(SWINPUT_SDL_UART1_IPP_UART_RXD_MUX, 0x01);	/* 20E_08FC *//* IOMUXC_UART1_UART_RX_DATA_SELECT_INPUT */
	 ...
	 ***********/
	/* SD4 CLK */
	set_mux_cfg(SWMUX_SDL_SD4_CLK, MUX_CTL_MUX_MODE_ALT0);		/* 20E_0338 */
	set_pad_cfg(SWPAD_SDL_SD4_CLK, MX6Q_PAD_SETTINGS_USDHC);	/* 20E_0720 */
...
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/ipl/boards/mx6x-sabreARD/init.c $ $Rev: 814674 $")
#endif
