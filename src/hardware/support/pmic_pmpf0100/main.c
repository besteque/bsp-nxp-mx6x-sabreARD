...
int main(int argc, char *argv[])
{
    ...
    result = pmpf0100_comms_set_bus_speed(info.pPmicComms, info.bus_speed);
    if (result != EOK)
    {
    	fprintf(stderr, "  ERROR: %s cannot set PMIC comms speed (code %d)\n",
    			argv[0], result);
...
int decode_command_line(int argc, char *argv[])
{
    ...
    info.bus_speed = PMPF0100_MAX_2WIRE_BUS_SPEED / 2;  // 200000Hz
    ...
    while (!done)
    {
        ...
        switch (c)
        {
        	case -1:
        		...
        		break;
        	case 'b':
        		info.bus_number = strtoul(optarg, &optarg, 0);
        		break;
        	case 'a':
        		info.bus_address = strtoul(optarg, &optarg, 0);
        		break;
        	case 'i':
        		info.irq_number = strtoul(optarg, &optarg, 0);
        		break;
        	case 's':
        		info.bus_speed =  strtoul(optarg, &optarg, 0);
        		break;
        	case 'v':
        		info.verbosity =  strtoul(optarg, &optarg, 0);
        		break;
        	default:
    ...
    if (info.verbosity > 0)
    {
    	...
    	fprintf(stderr, "  I2C bus speed .....: %dHz\n", info.bus_speed);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/support/pmic_pmpf0100/main.c $ $Rev: 809180 $")
#endif
