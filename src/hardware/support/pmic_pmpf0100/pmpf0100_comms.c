...
int pmpf0100_comms_get_default_irq(PMPF0100_COMMS *pComms, int *irqNumber)
{
	...
	// TODO read the QNX syspage entry to get this info
...
int pmpf0100_comms_irq_process(PMPF0100_COMMS *pComms, unsigned int timeout)
{
	...
	// TODO: temporary delay, remove for release and re-test!
	delay(10);
		                        ...
		                        uint32_t write_data, uint32_t *read_data)
{
	...
	if ((pComms == NULL) || (write_data == 0) || (read_data == NULL))
	{
		return EINVAL;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/support/pmic_pmpf0100/pmpf0100_comms.c $ $Rev: 809180 $")
#endif
