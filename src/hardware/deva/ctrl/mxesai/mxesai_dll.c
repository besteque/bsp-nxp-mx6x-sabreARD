	...
	while ((sample_offset * SAMPLE_SIZE) < size)
	{
		for (cnt = 0, slot_cnt = 0; cnt < mx->num_tx_aif; cnt++ )
		{
			if (mx->aif[cnt].play_strm.active)
			{
				...
				if (((mx->aif[cnt].play_strm.pcm_offset * SAMPLE_SIZE) %
	...
	while ((sample_offset * SAMPLE_SIZE) < size)
	{
		for (cnt = 0, frame_offset = 0; cnt < mx->num_rx_aif; cnt++ )
		{
			if (mx->aif[cnt].cap_strm.active)
			{
				...
				if (((mx->aif[cnt].cap_strm.pcm_offset * SAMPLE_SIZE) %
...
mxesai_cap_pulse_hdlr (HW_CONTEXT_T * mx, struct sigevent *event)
{
	...
	else
	{
		...
		mxesai_dmacapture_split(mx, (uint8_t*)
								(&mx->capture_dmabuf.addr[mx->capture_frag_index *
														  (mx->capture_dmabuf.size / 2)]),
								(mx->capture_dmabuf.size / 2));
		mx->capture_frag_index++;
	}
}
...
mxesai_init (HW_CONTEXT_T * mx)
{
	...
	ado_debug(DB_LVL_DRIVER, "%s: mclk = %d, sample rate = %d, nslots = %d, slot size = %d",
			  __FUNCTION__, mx->main_clk_freq, mx->sample_rate, mx->nslots, mx->slot_size);
...
int mxesai_parse_commandline(HW_CONTEXT_T * mx, char *args)
{
	...
	if (mx->i2c_bus != -1)
	{
		if (strcmp (mx->mixeropts, "") != 0)
			strlcat (local_string, ":", LOCAL_STR_LEN);
		snprintf(local_string, LOCAL_STR_LEN, "%sdev=%d", local_string, mx->i2c_bus);
	}

	if (mx->i2c_addr != -1)
	{
		if (strcmp (local_string, "") != 0)
			strlcat (local_string, ":", LOCAL_STR_LEN);
		snprintf(local_string, LOCAL_STR_LEN, "%saddr=0x%x", local_string, mx->i2c_addr);
	}

    if (strcmp (local_string, "") != 0)
        strlcat (local_string, ":", LOCAL_STR_LEN);
	snprintf(local_string, LOCAL_STR_LEN, "%srx_voices=%d", local_string, mx->aif[0].cap_strm.voices);

	strlcat(mx->mixeropts, local_string, MAX_MIXEROPT);
...
ctrl_init (HW_CONTEXT_T ** hw_context, ado_card_t * card, char *args)
{
	...
	*hw_context = mx;
	...
	if(mx->num_rx_aif > 1)
	{
		...
		int alignment = SAMPLE_SIZE * mx->nslots * mx->sdi_pins;
		...
		if((mx->capture_dmabuf.addr = ado_shm_alloc(mx->capture_dmabuf.size, mx->capture_dmabuf.name,
                                                    mx->capture_dmabuf.flags, &mx->capture_dmabuf.phys_addr)) == NULL)
		{
			ado_error ("MX ESAI: Unable to initialize ESAI registers");
			ctrl_destroy(mx);
			return -1;
	...
	if(mx->num_tx_aif > 1)
	{
		...
		if((mx->playback_dmabuf.addr = ado_shm_alloc(mx->playback_dmabuf.size, mx->playback_dmabuf.name,
                                                     mx->playback_dmabuf.flags, &mx->playback_dmabuf.phys_addr)) == NULL)
		{
			ado_error ("MX ESAI: Unable to initialize ESAI registers");
			ctrl_destroy(mx);
			return -1;
	...
	for (cnt = 0; cnt < max(mx->num_tx_aif, mx->num_rx_aif); cnt++)
	{
		...
		ado_debug(DB_LVL_DRIVER, "MX ESAI: PCM%d -> TX voices = %d, RX voices = %d", cnt,
				  mx->aif[cnt].play_strm.voices, mx->aif[cnt].cap_strm.voices);
		if (cnt < mx->num_tx_aif) {
			codec_set_default_group(mx->aif[cnt].pcm, mx->mixer, ADO_PCM_CHANNEL_PLAYBACK, cnt);
		}
		if (cnt < mx->num_rx_aif) {
			codec_set_default_group(mx->aif[cnt].pcm, mx->mixer, ADO_PCM_CHANNEL_CAPTURE, cnt);
		}
	}
	return 0;
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/deva/ctrl/mxesai/mxesai_dll.c $ $Rev: 811956 $")
#endif
