...
{
	cs42448_context_t *cs42448 = ado_mixer_get_context(mixer);
	...
	switch (channel)
	{
		case ADO_PCM_CHANNEL_PLAYBACK:
			ado_pcm_chn_mixer (pcm, ADO_PCM_CHANNEL_PLAYBACK, mixer,
							   ...
							   ado_mixer_find_group (mixer, SND_MIXER_PCM_OUT, index));
			break;
		case ADO_PCM_CHANNEL_CAPTURE:
			ado_pcm_chn_mixer (pcm, ADO_PCM_CHANNEL_CAPTURE, mixer,
							   ...
							   ado_mixer_find_group (mixer, cs42448->cap_strm_voices == 4 ? SND_MIXER_PCM_IN : SND_MIXER_LINE_IN, index));
			break;
		default:
			break;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/deva/codecs/cs42448/cs42448.c $ $Rev: 816773 $")
#endif
